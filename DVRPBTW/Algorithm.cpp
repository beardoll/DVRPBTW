...
void Algorithm::shawRemoval(vector<Car*> &originCarSet, vector<Customer*> &removedCustomer,
...
void Algorithm::randomRemoval(vector<Car*> &originCarSet, vector<Customer*> &removedCustomer, int q){
	// originCarSet: 未执行remove操作前的货车集合
	// removedCarSet: 执行remove操作后的货车集合
	// removedCustomer: 被移除的顾客节点
	// q: 本次需要remove的顾客数量
...
void Algorithm::worstRemoval(vector<Car*> &originCarSet, vector<Customer*> &removedCustomer, int q, int p){
	// originCarSet: 未执行remove操作前的货车集合
	// removedCarSet: 执行remove操作后的货车集合
	// removedCustomer: 被移除的顾客节点
	// q: 本次需要remove的顾客数量
...
void Algorithm::greedyInsert(vector<Car*> &removedCarSet, vector<Customer*> removedCustomer,
...
void Algorithm::regretInsert(vector<Car*> &removedCarSet, vector<Customer*> removedCustomer,
							 float noiseAmount, bool noiseAdd){
	// 把removedCustomer插入到removedCarSet中
	int removedCustomerNum = removedCustomer.end() - removedCustomer.begin();  // 需要插入到路径中的节点数目
	int carNum = removedCarSet.end() - removedCarSet.begin();  // 车辆数目
...
void Algorithm::reallocateCarIndex(vector<Car*> &originCarSet){  // 重新为货车编号
	for(int i=0; i<(int)originCarSet.size(); i++){
		originCarSet[i]->changeCarIndex(i);
	}
}

void Algorithm::removeNullRoute(vector<Car*> &originCarSet){    // 清除空车辆 
...
void Algorithm::run(vector<Car*> &finalCarSet, float finalCost){  // 运行算法，相当于算法的main()函数
	int i;
